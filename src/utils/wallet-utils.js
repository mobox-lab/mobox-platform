...
// Vuex store module: feat: ✨ add game replay functionality
export const feat____add_game_replay_functionalityModule = {
  namespaced: true,
  state: {
    items: [],
    loading: false,
  ...
  actions: {
    async fetchItems({ commit }, params = {}) {
      ...
      try {
        const response = await api.get('/feat____add_game_replay_functionality', { params });
